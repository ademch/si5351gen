...
Si5351::Si5351(uint8_t i2c_addr) : m_i2c_bus_addr(i2c_addr)
{
	m_xtal_freq = SI5351_XTAL_FREQ;
	bIsOnline   = false;
}
...
bool Si5351::init(uint8_t xtal_load_c, int32_t corr)
{
	...
	Wire.beginTransmission(m_i2c_bus_addr);
...
/*
 * Sets the clock frequency of the specified clkChannel.
...
uint8_t Si5351::set_freq(enum si5351_clock clkChannel, uint64_t freqFP)
{
	...
	if (clkChannel <= SI5351_CLK5)
	{
		// Lower bounds check
		if (freqFP < SI5351_CLKOUT_MIN_FREQ * SI5351_FREQ_MULT)
			freqFP = SI5351_CLKOUT_MIN_FREQ * SI5351_FREQ_MULT;

		// Upper bounds check
		if (freqFP > SI5351_MULTISYNTH_MAX_FREQ * SI5351_FREQ_MULT)
			freqFP = SI5351_MULTISYNTH_MAX_FREQ * SI5351_FREQ_MULT;
		...
		if (freqFP > SI5351_MULTISYNTH_SHARE_MAX * SI5351_FREQ_MULT)
		{
			// Check other clocks on same PLL
			for (uint8_t i = 0; i < 6; i++)
			{
				if (i == clkChannel) continue;

				if (m_clk_freqFP[i] > SI5351_MULTISYNTH_SHARE_MAX * SI5351_FREQ_MULT)
				{
					if (pll_assigned_to_clk[i] == pll_assigned_to_clk[clkChannel])
			...
			multisynth_calc_p1p2p3(freqFP, pll_freq, &ms_reg);

			// Set multisynth registers
			set_multisynth_reg(clkChannel, ms_reg, SI5351_OUTPUT_CLK_DIV_1);

			// Set PLL
			set_pll_freq(pll_assigned_to_clk[clkChannel], pll_freq);

			// Reset the PLL
			pll_reset(pll_assigned_to_clk[clkChannel]);

			// Enable the output on first set_freq only
			if (!m_clk_is_set[clkChannel])
			{
				set_clock_enable(clkChannel, true);
				m_clk_is_set[clkChannel] = true;
			}

			// Set the freq in memory
			m_clk_freqFP[clkChannel] = freqFP;
			...
			for (uint8_t i = 0; i < 6; i++)
			{
				...
				if ( (m_clk_freqFP[i] != 0) && (pll_assigned_to_clk[i] == pll_assigned_to_clk[clkChannel]) )
				{
						// Select the proper R div value
						uint64_t temp_freq = m_clk_freqFP[i];
		...
		else  // requested freqFP < 100 MHz
		{
			// Select the proper R div value
			uint8_t r_div = select_RDIV_forLowFreq(freqFP);

			// Calculate the synth parameters
			struct Si5351RegSet ms_reg = {};
			if (pll_assigned_to_clk[clkChannel] == SI5351_PLLA)
				multisynth_calc_p1p2p3(freqFP, m_plla_freqFP, &ms_reg);
			else
				multisynth_calc_p1p2p3(freqFP, m_pllb_freqFP, &ms_reg);

			// Set multisynth registers
			set_multisynth_reg(clkChannel, ms_reg, r_div);
...
// Set the clock frequency of the specified clkChannel calculating integer multisynth frequency.
// PLL frequency is calculated automatically
//
// clkChannel	- Clock output
// freqFP		- Output frequency in Hz*100 (fixed point xxx xxx xxx.xx)
uint8_t Si5351::set_freq_manual(enum si5351_clock clkChannel, uint64_t freqFP)
{
	if (!bIsOnline) return 10;
	...
	// Upper bounds check
	if (freqFP > SI5351_CLKOUT_MAX_FREQ * SI5351_FREQ_MULT)
		freqFP = SI5351_CLKOUT_MAX_FREQ * SI5351_FREQ_MULT;
...
// pll_freqFP	- Desired PLL frequency in Hz*100 (fixed point xxx xxx xxx.xx)
//
void Si5351::set_pll_freq(enum si5351_pll targetPLL, uint64_t pll_freqFP)
{
	if (!bIsOnline) return;

	struct Si5351RegSet pll_reg = {};
	pll_calc_p1p2p3(pll_freqFP, &pll_reg, m_ref_correction);
 ...
 * r_div		- Desired r_div
 */
void Si5351::set_multisynth_reg(enum si5351_clock clkChannel, struct Si5351RegSet ms_reg, uint8_t r_div)
{
	uint8_t *params = new uint8_t[8];
		...
		if (clkChannel <= SI5351_CLK5)
		{
			...
			// Register 44 (CLK0)
			uint8_t reg_val = si5351_read((SI5351_CLK0_PARAMETERS_ADDR + 2) + (clkChannel * 8));
			reg_val &= ~0x03;
			...
			// Write the parameters
			si5351_write_bulk(SI5351_CLK0_PARAMETERS_ADDR + 8 * (uint8_t)clkChannel, 8, params);

			set_multisynth_integer_mode(clkChannel, ms_reg.bIntMode);
			set_multisynth_rdiv(clkChannel, r_div, ms_reg.bDivBy4);
		}
		else
		{
			...
			// Write the parameters
			si5351_write(SI5351_CLK6_PARAMETERS_ADDR + (uint8_t)clkChannel - 6, params[0]);
			set_multisynth_rdiv(clkChannel, r_div, ms_reg.bDivBy4);
		}
...
void Si5351::set_clock_enable(enum si5351_clock clkChannel, bool bEnable)
{
	if (!bIsOnline) return;
 ...
 */
void Si5351::drive_strength(enum si5351_clock clkChannel, enum si5351_drive drive)
{
	if (!bIsOnline) return;

	uint8_t reg_val = si5351_read(SI5351_CLK0_CTRL_ADDR + (uint8_t)clkChannel);

	reg_val &= ~0x03;
	reg_val |= (uint8_t)drive;

	si5351_write(SI5351_CLK0_CTRL_ADDR + (uint8_t)clkChannel, reg_val);
}
...
void Si5351::set_correction(int32_t corr)
{
	...
	set_pll_freq(SI5351_PLLA, m_plla_freqFP);
	set_pll_freq(SI5351_PLLB, m_pllb_freqFP);
}

/*
 * clkChannel	- Clock output (use the si5351_clock enum)
 ...
 */
void Si5351::set_phase(enum si5351_clock clkChannel, uint8_t phase)
{
	if (!bIsOnline) return;
	...
	si5351_write(SI5351_CLK0_PHASE_OFFSET + (uint8_t)clkChannel, phase);
}
...
//
void Si5351::set_multisynth_integer_mode(enum si5351_clock clkChannel, bool bEnable)
{
	uint8_t reg_val = si5351_read(SI5351_CLK0_CTRL_ADDR + (uint8_t)clkChannel);
 ...
 */
void Si5351::set_clock_powerON(enum si5351_clock clkChannel, bool bPowerUp)
{
	if (!bIsOnline) return;
 ...
 */
void Si5351::set_clock_invert(enum si5351_clock clkChannel, bool bInvert)
{
	if (!bIsOnline) return;
...
//
void Si5351::set_clock_source(enum si5351_clock clkChannel, enum si5351_clock_source src)
{
	if (!bIsOnline) return;
	...
	switch (src)
	{
		...
		case SI5351_CLK_SRC_MS_RELAY:
			if (clkChannel == SI5351_CLK0) return;		// not allowed combination for CLK0
			if (clkChannel == SI5351_CLK4) return;		// not allowed combination for CLK4
...
//
void Si5351::set_multisynth_source(enum si5351_clock clkChannel, enum si5351_pll pll)
{
	if (!bIsOnline) return;
	...
	pll_assigned_to_clk[clkChannel] = pll;
}
 ...
 */
void Si5351::set_clock_disabled_state(enum si5351_clock clkChannel, enum si5351_clock_disable dis_state)
{
	if (!bIsOnline) return;

	uint8_t reg_val, reg;

	if (clkChannel <= SI5351_CLK3)
	{
		reg = SI5351_CLK3_0_DISABLE_STATE;

		reg_val = si5351_read(reg);
		reg_val &= ~(0b11 << (clkChannel * 2));
		reg_val |= dis_state << (clkChannel * 2);
	}
	else if (clkChannel <= SI5351_CLK7)
	{
		reg = SI5351_CLK7_4_DISABLE_STATE;

		reg_val = si5351_read(reg);
		reg_val &= ~(0b11 << ((clkChannel - 4) * 2));
		reg_val |= dis_state << ((clkChannel - 4) * 2);
	}
	else
		return;
...
uint8_t Si5351::si5351_read(uint8_t addr)
{
	...
	Wire.requestFrom(m_i2c_bus_addr, (uint8_t)1);
...
uint64_t Si5351::pll_calc_p1p2p3(uint64_t pll_freqFP, struct Si5351RegSet *reg, int32_t correction)
{
	uint64_t ref_freq = m_xtal_freq * SI5351_FREQ_MULT;
	...
	// PLL bounds checking
	if (pll_freqFP < SI5351_PLL_VCO_MIN * SI5351_FREQ_MULT)
		pll_freqFP = SI5351_PLL_VCO_MIN * SI5351_FREQ_MULT;

	if (pll_freqFP > SI5351_PLL_VCO_MAX * SI5351_FREQ_MULT)
		pll_freqFP = SI5351_PLL_VCO_MAX * SI5351_FREQ_MULT;

	// Determine integer part of feedback equation
	uint32_t a, b, c;

	a = (uint32_t)(pll_freqFP / ref_freq);

	if (a < SI5351_PLL_A_MIN)
		pll_freqFP = ref_freq * SI5351_PLL_A_MIN;

	if (a > SI5351_PLL_A_MAX)
		pll_freqFP = ref_freq * SI5351_PLL_A_MAX;
	...
	// 15MHz * 1`000`000 / 25
	b = (uint32_t)(((pll_freqFP % ref_freq) * RFRAC_DENOM) / ref_freq);
	c = b ? RFRAC_DENOM : 1;
	...
	pll_freqFP =  ref_freq * a;
	pll_freqFP += ref_freq * b / c;
...
void Si5351::multisynth_calc_p1p2p3(uint64_t freqFP, uint64_t& pll_freqFP, struct Si5351RegSet *reg)
{
	// Multisynth bounds checking
	if (freqFP > SI5351_MULTISYNTH_MAX_FREQ * SI5351_FREQ_MULT)
		freqFP = SI5351_MULTISYNTH_MAX_FREQ * SI5351_FREQ_MULT;

	if (freqFP < SI5351_MULTISYNTH_MIN_FREQ * SI5351_FREQ_MULT)
		freqFP = SI5351_MULTISYNTH_MIN_FREQ * SI5351_FREQ_MULT;

	// If freqFP > 150 MHz, we need to use DIVBY4 and integer mode
	if (freqFP >= SI5351_MULTISYNTH_DIVBY4_FREQ * SI5351_FREQ_MULT)
	{
		reg->p1 = 0;
		reg->p2 = 0;
		reg->p3 = 1;
		...
		Serial.print("PLL freq fixed point: ");
		print_u64(pll_freqFP);
		Serial.println("");
	...
	else  // pll_freqFP is provided on input
	{
		// a+b/c = pll/freq ---> a = pll/freq, b/c = pll%freq ---> b = pll%freq * c

		// Determine integer part of feedback equation
		a = (uint32_t)(pll_freqFP / freqFP);

		if (a < SI5351_MULTISYNTH_A_MIN)
			freqFP = pll_freqFP / SI5351_MULTISYNTH_A_MIN;

		if (a > SI5351_MULTISYNTH_A_MAX)
			freqFP = pll_freqFP / SI5351_MULTISYNTH_A_MAX;

		b = (uint32_t)((pll_freqFP % freqFP * RFRAC_DENOM) / freqFP);
		c = b ? RFRAC_DENOM : 1;
	}
...
void Si5351::set_multisynth_rdiv(enum si5351_clock clkChannel, uint8_t r_div, bool bDivBy4)
{
	uint8_t reg_addr = 0;

	if (clkChannel <= SI5351_CLK5)
		reg_addr = SI5351_CLK0_PARAMETERS_ADDR + 2 + 8 * (uint8_t)clkChannel;
	else
	...
	else if (clkChannel == SI5351_CLK6)
	{
		// Clear the relevant bits
		reg_val &= ~0x07;
		reg_val |= r_div;
...
uint8_t Si5351::select_RDIV_forLowFreq(uint64_t& freqFP)
{
	if (freqFP < SI5351_CLKOUT_MIN_FREQ * SI5351_FREQ_MULT * 2)
	{
		freqFP *= 128ULL;
		return SI5351_OUTPUT_CLK_DIV_128;
	}
	else if (freqFP < SI5351_CLKOUT_MIN_FREQ * SI5351_FREQ_MULT * 4)
	{
		freqFP *= 64ULL;
		return SI5351_OUTPUT_CLK_DIV_64;
	}
	else if (freqFP < SI5351_CLKOUT_MIN_FREQ * SI5351_FREQ_MULT * 8)
	{
		freqFP *= 32ULL;
		return SI5351_OUTPUT_CLK_DIV_32;
	}
	else if (freqFP < SI5351_CLKOUT_MIN_FREQ * SI5351_FREQ_MULT * 16)
	{
		freqFP *= 16ULL;
		return SI5351_OUTPUT_CLK_DIV_16;
	}
	else if (freqFP < SI5351_CLKOUT_MIN_FREQ * SI5351_FREQ_MULT * 32)
	{
		freqFP *= 8ULL;
		return SI5351_OUTPUT_CLK_DIV_8;
	}
	else if (freqFP < SI5351_CLKOUT_MIN_FREQ * SI5351_FREQ_MULT * 64)
	{
		freqFP *= 4ULL;
		return SI5351_OUTPUT_CLK_DIV_4;
	}
	else if (freqFP < SI5351_CLKOUT_MIN_FREQ * SI5351_FREQ_MULT * 128)
	{
		freqFP *= 2ULL;
